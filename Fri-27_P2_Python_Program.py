...
def rotate_Base(autoclave_color):
    ...
    while(correct == False): #Loops until the QArm is infront of the correct autoclave
        ...
        #Once the correct autoclave is detected, exits the loop.
        if (cur_position[0] < autoclave_position[0]+0.01) and (cur_position[1] > autoclave_position[1]-0.01) and autoclave_color == "red": 
            correct = True
        elif (cur_position[0] < autoclave_position[0]+0.01) and (cur_position[1] < autoclave_position[1]+0.01) and autoclave_color == "green": 
            correct = True
        elif (cur_position[0] < autoclave_position[0]+0.01) and (cur_position[1] > autoclave_position[1]-0.01) and autoclave_color == "blue": 
            correct = True
        ...
        #Rotates the base according to the current potentiometer value (Ex: 25% is 90 degrees right from the starting position)
            ...
            if pot_right > 0.95 or pot_right < 0.05:
                rotate_percent = (pot_final - pot_initial)*260     #Determines the degrees that the base must rotate by taking the difference 
                                                                   #between the final and initial readings and multiplying by 360
                                                                   #If the potentiometer is less that 5% or above 95% multiply by 260 instead 
                                                                   #to prevent the QArm from rotating past the maximum range.
            else:
...
#Done by Alexis Fernandez
def drop_off(autoclave_color,container_Size):
    #Small/Large Autoclave Positions
